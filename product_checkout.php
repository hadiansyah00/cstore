          ...
          <div class="produk_detail">
            <form id="form_p_cart_checkout" action="javascript:snapPayprocess()" method="POST">
              <div class="row <?php if ($digitalonly=='y') { echo 'justify-content-center'; } ?>">
                <div class="col-xl-8 col-lg-7 mb-2 <?php if ($digitalonly=='y') { echo 'd-none'; } ?>">
              ...
              buttons: {
                ...
                Submit: function () {
                  ...
                  if (itipepay=='midtrans' && metodepembayaran=='bank'){
                    $.ajax({
                      url: '<?=$main_url;?>module/action.php?jen=snap_token_midtrans&idalamat='+alamatid+'&idkurir='+kurirdipilih+'&kodevoucher='+kodevoucher+'&digitalonly='+digitalonly,
                      cache: false,
      ...
      function prosescartCheckout(a,b,c,d,e,f,g){
        if (e=='y' || e=='p') {
          $.ajax({
            ...
            data :  { 'idalamat' : a, 'idkurir' : b, 'idunique' : c, 'snapobj' : d, 'statuspay' : e, 'kodevoucher' : f, 'potonganvoucher' : total_voucher, 'metodepembayaran' : g, 'digitalonly' : digitalonly },
            success: function (result) {
              console.log(result);
