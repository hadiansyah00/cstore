...
class Rest_proses_model extends CI_Model {
    ...
    public function proses_signin($postjson) {
        ...
        if ($postjson['option']==1) {
            ...
            $cektoken = $this->db->query("SELECT cust_ponsel FROM m_customer WHERE cust_ponsel='$postjson[email]'")->num_rows();
        }else{
        ...
        if ($cektoken>=1) {
            $cekdata = cekDatarowarray('m_customer',$fieldcust,$postjson['email']);
            ...
            if($cekdata['is_active']=='1'){
                if ($postjson['option']==1) {
                    ...
                    if ($resemail=='y') {
                        ...
                        return json_encode(array('success'=>false, 'msg'=>'Failed action, please refresh page.'));
                    }
                }
