...
class Rest_load_model extends CI_Model {
    ...
    public function load_riwayat_transaksi($postjson){
        ...
        $res = $this->db->query("SELECT a.*,b.kurir,b.nama_kurir,b.level_kurir,b.lama_pengiriman FROM tx_transaksi a LEFT JOIN tx_kurir b ON a.no_transaksi=b.no_transaksi WHERE a.cust_id='$postjson[idcust]' $wh ORDER BY a.transaksi_id DESC ")->result_array();
        foreach ($res as $rows) {
    ...
    public function load_voucher($postjson){
        $data = array();
        $dnow = date('Y-m-d');
        $query = $this->db->query("SELECT * FROM m_voucher WHERE status_voucher='y' AND is_hapus='n' AND date(tgl_akhir) > '$dnow' OR voucher_id=1");
    ...
    public function snap_token_midtrans($postjson,$rest_cart){
        ...
        $rest_cust = $this->db->query("SELECT a.*,b.cust_nama,b.cust_ponsel,b.is_token FROM m_customer_det a JOIN m_customer b ON a.cust_id=b.cust_id WHERE a.cust_id='$postjson[idcust]' AND a.cust_det_id='$postjson[idalamat]'")->row_array();
        ...
        if ($postjson['digitalonly']=='n') {
            $gross_amount = is_numeric($rest_cart['total_bayar'])+is_numeric($rest_kurir['ongkos_kirim']);
        }else{
            $gross_amount = is_numeric($rest_cart['total_bayar']);
        }
